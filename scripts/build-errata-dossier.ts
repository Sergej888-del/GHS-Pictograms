...
//
// Собирает досье по всем свидетельствам `annex6Errata.ts`: по каждому — акт-источник, ссылка на
...
for (const [lang, indexes] of [...byLang].sort()) {
  /**
   * ⚠⚠⚠ АНГЛИЙСКАЯ КОНСОЛИДАЦИЯ ЛЕЖИТ БЕЗ СУФФИКСА ЯЗЫКА — `clp-consolidated.html`.
   *
   * Ровно об эту грабль предупреждал разбор `claude/eurlex-truncated-names.md`
   * («при повторе замера легко пропустить»), и session 71 на неё наступила:
   * первое же свидетельство на английском выпало из досье молча, с одной
   * строкой `⚠ нет консолидации EN` в потоке вывода. До session 71 английских
   * свидетельств не было вовсе, поэтому промах и не проявлялся.
   */
  const consPath = lang.toUpperCase() === 'EN'
    ? join(TMP, 'clp-consolidated.html')
    : join(TMP, `clp-consolidated-${lang.toLowerCase()}.html`)
  /**
   * ⚠⚠⚠ ПРОПУСК ЯЗЫКА — ОСТАНОВКА, А НЕ ПРЕДУПРЕЖДЕНИЕ. Прежде здесь стоял
   * `continue`, и досье выходило короче модуля на пропущенные строки. Это не
   * косметика: страница `/compliance/clp-translation-errors/` собирается из
   * досье и сверяет его длину с `ERRATA_COUNT`, то есть сборка сайта упала бы
   * позже и в другом месте, а сообщение о причине осталось бы в этом выводе,
   * который никто уже не читает. Хуже того — пока длины случайно совпадают,
   * с сайта просто исчезает пометка, и он молча перестаёт предупреждать.
   */
  if (!existsSync(consPath)) {
    console.error(`⛔ нет консолидации ${lang}: искал ${consPath}`)
    console.error('   Досье НЕ записано: без неё свидетельства этого языка выпали бы молча.')
    process.exit(1)
  }
  ...
  for (const index of indexes) {
    ...
    // ⭐⭐⭐ ОТСУТСТВИЕ ФАКСИМИЛЕ — ТОЖЕ РАСХОЖДЕНИЕ, А НЕ ТИШИНА. Прежде сторож
    // молчал, когда PDF акта на этом языке не скачан: `page === null` просто
    // пропускало проверку. Значит номер полосы, вписанный по догадке (скажем,
    // «у соседних редакций такая же»), уезжал на живую страницу непроверенным
    // — а страница зовёт читателя открыть ОЖ и убедиться. Session 71: три
    // новых свидетельства по анилину, у MT и SV факсимиле было, у EN нет, и
    // соблазн поставить ту же полосу был ровно этот.
    if (want.page === null) {
      bad.push(`полоса ${e.source.page} НЕ ПРОВЕРЕНА — нет ${TMP}/act-${e.source.act}-${lang.toLowerCase()}.pdf`)
    }
    if (bad.length) {
      srcMismatch.push(`${lang} ${index}: ${bad.join(', ')}`)
    }

    // ⭐⭐ СТОРОЖ ВОССТАНОВЛЕНИЯ. `corrected` печатается КНОПКОЙ в конструкторе
    // этикетки: человек может выбрать это чтение и напечатать его на таре.
    // Значит оно обязано отличаться от того, что стоит в акте, — иначе
    // свидетельство устарело (или запись уже поправили), и кнопка предлагает
    // ровно то же самое, выдавая это за выбор.
    if (e.corrected && want.act && e.corrected === want.act) {
      srcMismatch.push(`${lang} ${index}: corrected «${e.corrected}» совпадает с текстом акта — свидетельство устарело`)
    }
...
const out: string[] = []
// ⚠ Число в заголовке СЧИТАЕТСЯ, а не вписано. Вписанное «29» пережило и
// тридцатую находку, и тридцать третью: заголовок досье, которое пойдёт в
// обращение, обязан совпадать с его же содержанием.
out.push(`# Досье: ${findings.length} ошибок в языковых редакциях Annex VI CLP`)
out.push('')
