...
/**
 * ⚠⚠ ПОСЛЕДНИЙ ЗНАК ИНДЕКСНОГО НОМЕРА — КОНТРОЛЬНЫЙ, И ОН БЫВАЕТ БУКВОЙ X.
 *
 * `001-003-00-X` (гидрид натрия), `006-006-00-X` (синильная кислота),
 * `005-001-00-X` (трифторид бора). Шаблон, кончавшийся на `\\d`, отбрасывал их
 * молча — во ВСЕХ 23 языках одинаково, поэтому сверка языков между собой
 * ничего не замечала: дыра была у всех в одном и том же месте.
 *
 * Цена замерена: **384 записи из 4 178 в нашей таблице substances (9,2 %)**,
 * то есть около 8 800 строк перевода на 23 языка.
 *
 * ⚠⚠ ЭТО ВТОРОЙ ДЕФЕКТ В ОДНОЙ И ТОЙ ЖЕ РЕГУЛЯРКЕ. Первым было длинное тире
 * (см. ниже), его починили в session 48 и контрольный знак не заметили.
 * Урок: строгий шаблон, снятый с одного примера, надо сверять со ВСЕМ
 * множеством значений, а не с тем, что попалось под руку.
 */
const INDEX_NO = /^\d{3}-\d{3}-\d{2}-[\dX]$/i;
...
function annex6Names(html) {
  ...
  for (const tbl of iterTables(html)) {
    for (const row of iterRows(tbl)) {
      /**
       * ⚠⚠ МЕТКИ ПРАВОК EUR-LEX СЪЕДАЮТ ЗАПИСЬ ЦЕЛИКОМ.
       *
       * Строка диоксида титана размечена так:
       *   ►C6 | | | 022-006-00-2 | ◄ | … | titanium dioxide; … |
       * то есть `cells[0]` — это «►C6», а не номер, и строгая проверка номера
       * не проходит. Запись выпадает МОЛЧА и ВО ВСЕХ 23 ЯЗЫКАХ сразу.
       *
       * ⚠ Чистка меток в этом файле уже была — но только в блоке сигнальных слов
       * (там словенское «Nevarno» приходило как «►C9 Nevarno ◄»). В блок имён её
       * не перенесли, и разбор терял записи, у которых была правка регламента.
       *
       * ⚠⚠ ЭТО ТРЕТИЙ ДЕФЕКТ ОДНОГО КЛАССА подряд: длинное тире (session 48),
       * контрольный знак X, теперь метки правок. Каждый раз строгий шаблон
       * отбрасывал верную запись, и каждый раз молча. Цена здесь особенно
       * заметная: 022-006-00-2 — это диоксид титана, одна из самых спорных и
       * самых обсуждаемых классификаций CLP.
       */
      const cells = rowCells(row).map(stripMarkers).filter(Boolean);
