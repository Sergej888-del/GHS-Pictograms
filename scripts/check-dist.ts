...
const CHECKS: Check[] = [
  ...
  /*
   * ⚠⚠ ЗДЕСЬ СТОЯЛА ПРОВЕРКА СТРОК РЕЛИЗА — И ЕЁ ЗДЕСЬ БЫТЬ НЕ МОЖЕТ (s84).
   * Она читала `data_release`, чтобы поймать нашу внутреннюю бухгалтерию
   * («sessions 76-79», «s79: …») в строках, которые печатает аудиторский отчёт.
   * Упала с `permission denied`: таблица ЗАКРЫТА для anon с session 78 (§15.1),
   * а этот скрипт ходит именно anon-ключом — и должен остаться таким.
   * ⭐ Правило переехало в саму базу: ограничение `data_release_no_internal_marks`
   * (`scripts/sql/84-release-strings-constraint.sql`) физически не даёт записать
   * такую строку — ни миграцией, ни руками в SQL Editor. Сторож там, где данные,
   * а не там, где его удобно было написать.
   */
