...
- **Related tools on this site:** [Label Maker](https://ghspictograms.com/label-constructor/), [ATE Calculator](/tools/ate-mixture-calculator/)
...
For mixture classification under CLP, including the strict cut-off values for new hazard classes, the [ATE Calculator on GHSSymbols.com](/tools/ate-mixture-calculator/) handles the additivity formula.
...
- **[ATE Calculator](/tools/ate-mixture-calculator/)** — Compute the Acute Toxicity Estimate for a mixture using the GHS additivity formula. The calculator handles the strict cut-off values that EU CLP applies and shows how the same mixture would be classified under different jurisdictions. Useful when reviewing a formulation for first-time market entry into a country with different mixture rules.
