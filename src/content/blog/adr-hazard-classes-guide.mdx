...
👉 Look the number up in the [UN number reference](/un/): the ADR 2025 row, the 49 CFR 172.101 row and the orange plate, each under its own source.

> **Looking for GHS hazard classes instead?** ADR classes govern the *transport* of dangerous goods by road, and there are 9 of them. The classification behind *labels and safety data sheets* is a separate system with 33 classes — see [all GHS hazard classes and categories](/hazard-classes/).
...
- 🔍 [UN number reference](/un/) — The ADR and US table entries for a number, side by side and never merged
